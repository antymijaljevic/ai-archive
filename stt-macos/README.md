# Voice-to-Text via Cursor/Clipboard with Global Hotkeys

Record voice with a keyboard shortcut and automatically send transcriptions to the active cursor position or clipboard.

## Features

- 🎙️ **Global hotkey** to start/stop recording (default: Right Shift ⇧)
- 🤖 **Auto-transcription** using Whisper (offline, fast)
- 📝 **Auto-inserts** text at the current cursor position or copies to clipboard (you can review first!)
...
You should see:
```
🎙️  Voice-to-Text via Cursor/Clipboard Ready
...
5. Transcription is inserted at your cursor position (review it, then press Enter to submit). It is also copied to your clipboard.
...
### Transcription not appearing at cursor / where expected
- Ensure an active text input field is focused.
- Transcription is inserted (not auto-submitted) - you might need to press Enter or take further action.
- Check your clipboard; the text is always copied there as a fallback.
